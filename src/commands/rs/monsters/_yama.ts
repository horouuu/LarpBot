...
const SupplyDrop = new LootTable()
  .tertiary(
    1,
    new LootTable().add("Pineapple pizza", [3, 4]).add("Wild pie", [3, 4])
  )
  .tertiary(
    1,
    new LootTable().add("Prayer potion(3)", 2).add("Super restore mix(2)", 2)
  )
  .tertiary(
    1,
    new LootTable().add("Super combat potion(1)", 1).add("Zamorak mix(2)", 1)
  );
...
const YamaTable = new LootTable()
  ...
  .add("Dossier", 1, 750)
  .add("Oathplate shards", 12, 515)
  .add("Forgotten lockbox", 1, 303)
  .add("Soulflame horn", 1, 30)
  .add("Oathplate helm", 1, 15)
  .add("Oathplate chest", 1, 15)
  .add("Oathplate legs", 1, 15)
